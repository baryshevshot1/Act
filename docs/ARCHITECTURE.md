...
Шесть готовых Level C: **Identity & Auth**, **Events (с Recurrence слитой внутрь)**, **RSVP & Attendance** (три критичных, реализуются end-to-end в пилоте) + **Contacts Sharing**, **Recommendations**, **Localization** (добавлены в P0 Wave 1, лёгкое покрытие). Level C для оставшихся 10 контекстов — после Pilot Этапа 0 (см. Phase 4 в roadmap).
...
|`ConsentRecord`     |`id`, `user_id`, `purpose` (`core_processing`, `marketing`, `contact_sharing`), `consent_text_hash`, `granted_at`, `withdrawn_at`, `ip_address`|
...
**Per-task SKILL.md** (в `.claude/skills/`) — для повторяющихся операций: «Добавить новый bounded context», «Написать RLS policy», «Создать миграцию с zero downtime», «Добавить OG-картинку для новой entity». **Текущие skills (Iteration 4):** `.claude/skills/write-rls-policy/SKILL.md` + `.claude/skills/outbox-event/SKILL.md`; оставшиеся 8 SKILL.md планируются в Iteration 6 (см. roadmap в `docs/iterations/` + `prompts/`).
...
- Compliance choices (`cross_border_consent` — это `ConsentRecord` row, не flag).
...
**Storage:** таблица `notifications_delivery (id, user_id, notification_type, channel, state, delivery_metadata jsonb, created_at, sent_at, delivered_at, acknowledged_at, expired_at, failed_at, bounced_at, suppressed_at)`. RLS: только owner. Подробная схема — в Level C Notifications post-pilot (текущий Level C status: **missing**; см. § «16 BC ↔ Level C status» в `docs/iterations/iteration-4.5-audit-report.md`).
